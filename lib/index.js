...
function apply(ctx, config) {
  ...
  async function sendGeneratingMessage(session, context) {
    const messageIds = [];
    await recallWizardMessages(session, context);
    let content = "\u{1F3B5} \u97F3\u4E50\u751F\u6210\u4E2D...\n\n";
    ...
    try {
      ...
      if (config.outputFormat === "hex") {
        ...
        if (config.sendAsFile) {
          const safeTitle = title || context?.generatedTitle || "music";
          const safeFilename = safeTitle.replace(/[\/\\:*?"<>|]/g, "_").substring(0, 50);
          const filename2 = `${safeFilename}.${ext}`;
...
\u8F93\u5165 "\u53D6\u6D88" \u53EF\u7EC8\u6B62\u64CD\u4F5C`
    ...
    return `\u2753 \u65E0\u6548\u7684\u9009\u62E9\uFF0C\u8BF7\u91CD\u65B0\u8F93\u5165\uFF08\u5269\u4F59 ${2 - context.invalidInputCount} \u6B21\uFF09:
